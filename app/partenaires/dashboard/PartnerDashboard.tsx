...
export default function PartnerDashboard({
  ...
  const filteredProfiles = useMemo(() => {
    ...
    selectedPhases, selectedNeedCats, selectedSkillCats,
  ...
  function handleRoleFilter(v: 'all' | 'entrepreneur' | 'talent') {
    setRoleFilter(v);
    setPage(1);
    if (v === 'talent')       { setSelectedPhases([]); setSelectedNeedCats([]); }
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-4 mb-6 flex flex-col gap-4">
        ...
        {/* Phase filter — porteurs */}
        {showEntrepreneurFilters && (
          <div>
            <p className="text-xs font-medium text-neutral-500 mb-2">
              Phase du projet
              {selectedPhases.length > 0 && (
                <button onClick={() => { setSelectedPhases([]); setPage(1); }} className="ml-2 text-primary-600 hover:underline font-normal">
                  Tout voir
                </button>
              )}
            </p>
            <div className="flex flex-wrap gap-1.5">
              {PHASE_OPTIONS.map((opt) => (
                <button
                  key={opt.code}
                  onClick={() => toggle(selectedPhases, setSelectedPhases, opt.code)}
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                    selectedPhases.includes(opt.code)
        ...
        {/* Need categories filter — porteurs */}
        {showEntrepreneurFilters && (
          <div>
            <p className="text-xs font-medium text-neutral-500 mb-2">
              Catégories de besoins
              {selectedNeedCats.length > 0 && (
                <button onClick={() => { setSelectedNeedCats([]); setPage(1); }} className="ml-2 text-primary-600 hover:underline font-normal">
                  Tout voir
                </button>
              )}
            </p>
            <div className="flex flex-wrap gap-1.5">
              {NEED_CATEGORY_OPTIONS.map((opt) => (
                <button
                  key={opt.code}
                  onClick={() => toggle(selectedNeedCats, setSelectedNeedCats, opt.code)}
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                    selectedNeedCats.includes(opt.code)
                      ? 'bg-orange-500 text-white border-orange-500'
                      : 'border-neutral-200 text-neutral-600 hover:border-orange-300'
                  }`}
                >
        ...
        {/* Skill categories filter — talents */}
        {showTalentFilters && (
          <div>
            <p className="text-xs font-medium text-neutral-500 mb-2">
              Catégories de compétences
              {selectedSkillCats.length > 0 && (
                <button onClick={() => { setSelectedSkillCats([]); setPage(1); }} className="ml-2 text-primary-600 hover:underline font-normal">
                  Tout voir
                </button>
              )}
            </p>
            <div className="flex flex-wrap gap-1.5">
              {SKILL_CATEGORY_OPTIONS.map((opt) => (
                <button
                  key={opt.code}
                  onClick={() => toggle(selectedSkillCats, setSelectedSkillCats, opt.code)}
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                    selectedSkillCats.includes(opt.code)
